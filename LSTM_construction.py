...
class CTLSTM(nn.Module):
    ...
    def MC_Loss(self, times, tMax, Clows, Cbars, deltas, OutGates, Nsamples=1000):
        ...
        # Liste pour sauvegarder les losses individuelles par séquence
        individual_mc_losses = [I[seq_idx].item() for seq_idx in range(N_batch)]
        
        # return trands and the indices also for testing the integral + individual losses
        return pt.sum(I, dim=0)/N_batch, trands, t_up, individual_mc_losses
    ...
    def logLoss(self, seq, mask, lambOuts):
        ...
        # Calculer la loss pour chaque séquence individuellement
        for batch_idx in range(N_batch):
            seq_loss = 0.
            
            for ev in range(N_events):
                if mask[batch_idx, ev]:  # Seulement si l'événement est valide pour cette séquence
                    # Get the log of the intensity of the event that has occurred
                    lamb = lambOuts[batch_idx, ev, seq[batch_idx, ev]]
                    logLamb = pt.log(lamb)
                    seq_loss += logLamb
            
            # Sauvegarder la loss négative pour cette séquence
            individual_log_losses.append(-seq_loss.item())
            loss += seq_loss
        
        # return the batch average + individual losses
        return -loss/N_batch, individual_log_losses
